...
## 2. 一致性模型判断的对象是 History

### 2.1 一次操作不是一个点，而是一段时间区间
...
## 3. Real-time precedence 不是比较墙钟时间戳
...
### 4.2 Linearization point 是证明工具，不一定是一行固定代码
...
### 4.4 线性一致是 safety，不是 latency 或 availability 承诺
...
## 7. 可串行化：关心的是整个事务的等价执行

### 7.1 Serial 不等于 Serializable 的实现方式
...
### 8.1 External consistency 要看产品定义
...
## 9. Linearizable 绝不等于 Serializable
...
## 10. Snapshot Isolation 很有用，但不是 Serializability
...
## 12. Causal consistency：必须先看见原因，再看见结果
...
## 13. Eventual consistency 只承诺停止写入后的收敛
...
### 17.2 Serializable 也不必依赖分布式共识

单机数据库可以用锁或 MVCC 提供可串行化事务，却没有任何跨节点共识。

这再次说明二者不在同一个定义维度。

### 17.3 2PC 不是 Consensus
...
- 是否写出抽象对象、顺序规范、原子单位和保证范围？
- 是否需要现实时间顺序，还是只保留程序顺序或因果关系？
- 默认读取是否允许陈旧，界限按时间、版本还是会话定义？
- 多对象读取是否来自同一快照，minimum version 无法满足时是否静默降级？

### 写入、事务与故障

- ACK 的持久化和复制边界是什么，超时是否为结果未知？
- Serializable 是否还承诺 strict serializability？
- Snapshot Isolation 是否允许业务不变量发生 write skew？
- serialization failure 能否从事务起点安全重试，外部副作用如何隔离？
- 分区时哪些操作等待、拒绝或降级，旧 Leader 如何 fencing？
- Session token 与公开版本在故障转移、备份恢复后会不会倒退？

### 验证

- History 是否区分 invocation、response、fail 与 unknown？
- 是否分别检查单对象 linearizability 与事务隔离？
- 是否注入成员变更、进程暂停、只读副本落后和网络分区？
- 通过一次测试后，是否仍保留模型证明、代码审查和生产监控？

## 25. 一张决策表：业务到底需要哪一种保证
