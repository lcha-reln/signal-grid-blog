...
这三个词经常一起出现在多副本设计里，却回答三个不同问题：

| 问题       | 判断对象                                 | 可证伪的反例                                             |
| ---------- | ---------------------------------------- | -------------------------------------------------------- |
| 一致性模型 | 客户端调用与响应组成的 History           | 一个观察结果无法放进模型允许的顺序或依赖关系             |
| 收敛性     | 停止更新、通信恢复后各副本状态的最终关系 | 所有更新已经传播，两个可达副本仍永久得到不同逻辑状态     |
| 冲突解决   | 对并发版本的保留、拒绝、排序或合并规则   | 相同冲突事实在不同副本得到不同裁决，或裁决破坏业务不变量 |

三者不会互相推出。带稳定平局规则的 Last-Writer-Wins 可以让收到同一版本集合的副本选出同一结果，却可能因为时钟、覆盖写或丢失增量而违反业务语义，也不会因此获得线性一致。满足自身传输与代数前提的 CRDT 可以提供副本收敛，却不自动保留非重叠操作的现实时间顺序，也不自动保护任意跨对象不变量。反过来，一个线性一致服务约束的是已完成调用的合法 History；定义本身并不要求每个离线副本都在某个期限内追上权威状态。

因此契约要分三层写：读写 History 允许什么；故障停止后在什么传播前提和时限内收敛；并发更新由谁按哪个版本化规则裁决。把“最终会一样”当成冲突语义，会得到确定但错误的余额或库存；把“有冲突合并”当成一致性模型，又无法回答用户是否会读倒退、写是否会丢失，以及哪个 History 才算违规。
