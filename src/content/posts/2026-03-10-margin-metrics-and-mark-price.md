...
本文是交易系统学习路径的 Chapter 09。[Chapter 07：永续资金费用](/signal-grid-blog/posts/perpetual-funding-rate/) 会改变账户现金与风险缓冲，[Chapter 08：交易账本与双重记账](/signal-grid-blog/posts/trading-ledger-double-entry-accounting-and-reconciliation/) 已把这些现金流拆成 posted、pending 与 available 等可审计状态；这里再把账本余额和仓位估值接入同一风险快照。
...
- 金额、数量、价格和比率是否使用明确精度与舍入规则；
- 每次计算能否追溯到 position/order sequence、price snapshot 和 rule version；
- equity 中的 UPL 是否被重复加减；
- collateral haircut、liability、order loss 和费用缓冲是否各只应用一次；
- IM 与 MM 是否按当前名义价值和正确档位重算；
- 比率名称是否携带 numerator、denominator 与比较方向；
- stale/outlier price source 的降级是否有确定状态；
- UI estimate 与权威 liquidation decision 是否明确分开；
- 风险动作是否幂等，并能在重启后从相同输入恢复；
- 是否用边界值、档位跃迁、价格跳空和多资产联动做确定性回放测试。
