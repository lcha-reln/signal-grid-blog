...
本文是“有状态系统可靠性”专题的 Chapter 20，也是整条路径的 capstone。前面的 [WAL](/signal-grid-blog/posts/write-ahead-log-durability-and-crash-recovery/) 给出了本地 crash boundary，[Raft](/signal-grid-blog/posts/raft-consensus-leader-election-log-replication-and-safety/) 给出了提交前缀，[一致性模型](/signal-grid-blog/posts/consistency-models-linearizability-serializability-and-real-time-order/) 定义了合法 History，[分布式快照](/signal-grid-blog/posts/distributed-snapshots-consistent-checkpoints-barriers-recovery-cursors/) 约束状态与恢复游标，[Recovery Frontier](/signal-grid-blog/posts/history-retention-recovery-frontier-log-truncation-dedup-gc/) 定义安全回收边界，[备份与灾难恢复](/signal-grid-blog/posts/backup-pitr-disaster-recovery-and-restore-drills/) 定义 RPO/RTO，[静默损坏修复](/signal-grid-blog/posts/silent-data-corruption-checksum-scrubbing-isolation-authoritative-repair/) 补上完整性故障，[滚动升级](/signal-grid-blog/posts/stateful-system-rolling-upgrades-protocol-snapshot-migration-safe-rollback/) 引入 mixed-version History，[协议可观测性](/signal-grid-blog/posts/stateful-system-observability-epoch-commit-lag-cursor-recovery/) 规定运行时必须保留哪些状态与恢复证据，[TLA+ 与 Refinement](/signal-grid-blog/posts/protocol-pseudocode-to-tla-invariants-counterexamples-refinement/) 则把关键不变量和状态转移写成可探索规格。本章把这些协议边界变成可执行、可重放、可审计的实现验证方法。
