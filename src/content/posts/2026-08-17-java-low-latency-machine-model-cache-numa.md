...
只报告“对象有多少个”不能解释缓存行为。
## 3. Cache Coherence 不等于内存顺序，更不等于 JMM
...
## 4. Store Buffer：为什么写入能继续，别人却还没看见
...
### 4.1 Store buffer 与写合并并不是免费的无限队列
...
## 5. True Sharing 与 False Sharing
...
```
### 5.1 True sharing：先改协议，再谈 padding
...
### 5.2 False sharing：不同地址仍会争同一所有权
看似没有共享：
...
- GC 与分配率变化；
- 同一负载下的硬件计数器。
## 7. Hardware Prefetch：连续访问的朋友，也可能是噪声放大器
...
6. 最后才实验软件预取。
## 8. Branch Prediction：不要把所有 `if` 都改成位运算
...
flowchart LR
  ...
  C -->|"否"| M["向更低缓存 / 内存取数"]
```
### 9.1 大 footprint 会同时影响 Cache 和 TLB
...
### 9.2 Page fault 与 TLB miss 不是一回事
...
```
### 10.1 First-touch 是结果，不是 Java 语法
...
### 10.3 CPU affinity 与 memory policy 必须成对设计
...
### 13.5 Cache 冷暖不能靠一次 `System.gc()` 定义
...
### 14.2 `cache-misses` 不一定等于 LLC demand-load miss
...
### 14.5 不要把 miss 直接乘 line size 当成内存带宽
...
## 15. `perf c2c`：从“怀疑伪共享”走到具体 Cache Line
...
flowchart LR
  ...
  F --> R
```
### 15.1 HITM 不是 false sharing 的自动证明
高 HITM 也可能是：
...
## 18. 测试清单：从单线程到故障场景
### 18.1 正确性测试
- 多 writer 竞争下无丢失、重复或越界；
- 发布 sequence 与 payload 的 happens-before 正确；
- 32/64 位、x86-64/AArch64 支持矩阵内行为一致；
- JIT 编译前后结果一致；
- 故障切换与重启不依赖线程本地临时状态；
- padding 或布局改变不影响序列化兼容性。
### 18.2 微基准
- 至少多个 fork；
- 保存 JVM 参数和完整环境；
- 控制线程 topology；
- 比较 true sharing、false sharing 和无共享基线；
- 控制工作集与访问分布；
- 同时报告 throughput 与延迟；
- 检查分配率与 GC；
- 复核 JIT 输出；
- 避免只挑最好的一次。
### 18.3 组件实验
- 真实 producer / consumer 数量；
- 与生产一致的消息大小；
- 正常流量、突发和饱和；
- 队列满与背压；
- 同核心、同 node、跨 node 对照；
- sibling 噪声；
- 定时器、日志和监控线程共同运行；
- 页首次触碰与稳态分别测试。
### 18.4 端到端实验
- 从计划到达到最终响应的延迟；
- accepted、completed、goodput、timeout、reject；
- p99.9、p99.99 与最大值；
- CPU、功耗、频率和容器 throttling；
- GC、safepoint、JIT 与 page fault；
- context switch、migration 与 NUMA placement；
- perf counters 与业务阶段对齐；
- 长时间运行后的漂移。
## 19. 生产运维清单
机器模型会因硬件更换、固件更新、内核升级、JDK 升级和容器策略改变而漂移。 因此性能配置不能只存在于某位工程师的 shell history。
### 19.1 部署清单
- [ ] CPU vendor、family、model、stepping 与 microcode 已记录；
- [ ] Socket、die、core、SMT 与 NUMA 拓扑已保存；
- [ ] 每级 Cache 的 size、line size 与 shared CPU list 已保存；
- [ ] BIOS 的 NUMA、SMT、频率和电源策略已版本化；
- [ ] kernel、perf 与 JDK 完整版本已记录；
- [ ] 容器 CPU quota、cpuset CPUs、cpuset mems 已记录；
- [ ] latency-sensitive 线程 affinity 已验证；
- [ ] sibling 上的工作负载符合隔离策略；
- [ ] memory policy 与初始化/预触页流程一致；
- [ ] THP / HugeTLB 状态符合测试基线；
- [ ] 节点内存不足时的失败或 fallback 行为已演练。
### 19.2 运行指标
至少关联观察：

- 业务 p50、p99、p99.9、p99.99；
- offered、accepted、completed 与 rejected rate；
- 每核心利用率，而不是只有全机平均 CPU；
- run queue、context switch、CPU migration；
- major / minor page fault；
- NUMA local / remote 相关指标；
- 内存带宽与 LLC / TLB 模型专用指标；
- GC pause、allocation rate、safepoint；
- 频率、温度与 throttling；
- 队列深度和 backpressure。
### 19.3 变更后必须重建基线
以下事件都应触发性能复验：
...
| 说法 | 问题 | 更准确的表达 |
| --- | --- | --- |
| “L1 是 32 KB，L3 全 Socket 共享” | 把某代 CPU 当规范 | 在目标机器枚举容量与共享范围 |
| “Cache Line 就是 64 字节” | 常见值不是 Java 保证 | 查询 `coherency_line_size` / CPU 文档 |
| “volatile 会刷新主内存” | 混淆 JMM 与物理路径 | volatile 建立 JMM 顺序，JIT 映射到 ISA |
| “CPU 有 MESI，所以 data race 没问题” | coherence 不等于 happens-before | 用 Java 同步建立正确性 |
| “两个线程写不同字段，没有共享” | 忽略 Cache Line 粒度 | 验证字段是否同 line、是否产生 HITM |
| “加 padding 就能解决竞争” | true sharing 仍存在 | 先判断业务所有权，再处理布局 |
| “SoA 一定比 AoS 快” | 忽略访问集合 | 按热字段和访问模式比较 |
| “顺序访问一定很快” | 可能带宽饱和或污染缓存 | 同时报命中、带宽与尾延迟 |
| “branchless 一定快” | 可能增加指令与依赖 | 看机器码、分支 MPKI 和业务结果 |
| “Huge Page 一定降低延迟” | 忽略分配、碎片与策略 | 先证明 TLB bound，再做对照实验 |
| “绑核就解决 NUMA” | 页面可能仍在远端 | affinity、memory policy、first-touch 联动 |
| “跨 NUMA 固定慢两倍” | 倍率依平台与负载 | 在目标 SKU 测 latency 和 bandwidth |
| “逻辑 CPU 数就是独立核心数” | 忽略 SMT 共享资源 | 读取 sibling topology，分别实验 |
| “cache-misses 就是 DRAM miss” | generic event 语义不足 | 查 PMU 定义和 model-specific metrics |
| “HITM 就是伪共享” | 也可能是真共享 | 查看 source、offset 与业务所有权 |
## 21. 最终方法：从所有权到证据
低延迟机器模型可以压缩成七个问题：
