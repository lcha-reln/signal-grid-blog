...
这里也划定一个有意保持的边界：本文只讨论 **NIC queue、IRQ/softirq、CPU 与 NUMA 页之间的数据路径所有权**。`futex` 实现证据、AQS `park/unpark`、锁持有者被抢占和优先级反转，归属于前述[线程竞争、AQS 与调度章节](/signal-grid-blog/posts/java-thread-contention-aqs-park-unpark-scheduling/)的等待与 owner 模型；PI futex、`SCHED_FIFO/RR` 则是继续下钻时才需核对的 Linux 实时机制，不能被当成 Java 锁的默认能力，也不在本文扩成调参附录。当证据显示延迟来自 blocked owner、run-queue wait 或锁唤醒链时，应切换到竞争模型，而不是继续给本章追加更多绑核和网络开关。
