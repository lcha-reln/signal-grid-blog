...
updated: 2026-08-17T15:22:25+08:00
tags:
  - AI Agent
  - Agent Runtime
  - Tool Calling
...
### 2.5 Agent Runtime：让一次 Run 可控地活下去
...
### 2.6 Agent Platform：让多个 Agent 共用正确基础设施
...
## 5. 什么时候用规则、RAG、Workflow 或 Agent
...
## 11. TradeOps Agent Lab：一条完整调查轨迹
...
| 反模式                     | 问题与修正                                                                     |
| -------------------------- | ------------------------------------------------------------------------------ |
| 把 Tool Loop 当完整 Agent  | `while(tool_calls)` 只完成协议循环；还需要状态、预算、恢复、Policy 和 Trace。  |
| 把 Prompt 当权限系统       | “不要访问其他租户”不是授权检查；租户 ID 来自服务端上下文，查询层必须强制过滤。 |
| Schema 严格就等于动作正确  | Schema 只能减少解析歧义，不能证明语义、权限、新鲜度或幂等性。                  |
| 暴露万能 SQL、Shell 或 URL | 它把数据、网络和执行权限压进难验证的字符串；应改成窄工具和服务端参数。         |
| 把 Memory 当业务数据库     | 摘要会丢信息、过期和被污染；业务事实必须从权威系统读取。                       |
| 对写工具统一“失败就重试”   | 超时可能是 `UNKNOWN`；没有幂等键和状态查询，重试会重复副作用。                 |
| 只测试最终答案             | 安全违规、错误工具、重复写和成本失控都可能被漂亮答案遮住；必须评测轨迹。       |
| 无上限追加上下文           | 旧证据、冲突文档和注入会累积；应按任务选择并标记来源。                         |
| 一开始就拆成多 Agent       | 多 Agent 增加委托、共享状态和 Eval 难度；先证明单 Agent 或 Workflow 的边界。   |
| 保存私有思维链作审计依据   | 审计应基于可观察提案、证据、结果、Policy 决策和状态迁移。                      |
