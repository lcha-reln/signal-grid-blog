...
### RecordingPos 与 durable position 的距离
...
### Extend：继续同一条 recording，不是拼接任意流
...
部署记录必须固定 archiveDir、markFileDir 与 link 的关系，并保证同一目录只有一个 Archive owner。每一代备份同时保存 Archive format、Aeron version、配置和完整目录关系；单个 `.rec` 不得脱离 Catalog 与同代元数据手工搬运。

#### 持久性合同

业务承诺必须写明 file/catalog sync level、`RecordingPos` 对应的确认级别，以及存储设备对 `force` 的实际保证。单机或整个故障域丢失时的 RPO/RTO，不能从一个 position counter 自动推出，必须由副本、冷备和恢复时间共同满足。

#### 保留与恢复合同

Retention cutoff 必须覆盖所有 consumer 与 replication checkpoint；detach 到 delete 之间保留可验证的冷却期。truncate、purge、compact 等破坏性动作需要审批、整代备份和隔离演练；最近一次完整恢复的介质、业务摘要和耗时则是合同仍然有效的证据。

## 结论：可恢复存储必须把目录代际、持久位置与保留动作作为同一份合同
