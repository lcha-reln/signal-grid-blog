...
先把经常混用的三种“证据”分开：**JMH 是机制证据，实验室负载是容量与交互证据，生产诊断是真实症状与适用性证据。** JMH 通过受控 fork、状态和输入比较某段代码；组件与端到端负载在可重复拓扑中施加到达率、突发、背压和故障；生产中的 histogram、JFR、队列和错误事件则回答真实流量下发生了什么。生产观测代表性最高，却混入部署、租户、依赖和硬件差异，不能仅凭相关性替代受控因果实验；反过来，JMH 控制力最强，也不能替生产 SLA 背书。

下面的四层表把“实验室负载”再拆成组件与完整系统，因为两者的边界、饱和点和失败语义不同：
...
四层之间不是“越上层越准确”，三种证据也不能互相冒充。微基准控制力强但代表性窄；实验室负载可以主动扫过饱和与恢复，却仍缺少生产长期漂移；生产数据代表性强但混杂因素多。最好的证据是：JMH 解释机制，实验室负载验证外推和安全边界，生产诊断确认真实收益并触发下一轮可证伪实验。
...
### 用 JFR、async-profiler 与 perf 解释原因
...
#### async-profiler：看 Java、native 与 wall time
