...
**Active-Passive 只是有状态复制的一种模式，不是高可用的定义。**本文给出一种可落地的参考模型：**Kafka 分区日志 + 单写者状态机 + 热备 Follower + ZooKeeper 选主 + durable snapshot**。它偏向一致性；当节点无法证明自己仍是合法 Leader 时，正确行为是停止写入，而不是带着不确定性继续服务。需要分区两侧都继续写、已确认写跨区域零丢失或跨 shard 原子提交的系统，必须选择不同的复制与事务协议，不能从“有两个实例”推导这些性质。
...
## Active-Passive 只是一个实现点，复制协议由 ACK 与分区语义决定

“主备”“双活”“多副本”描述的是部署外形，不能单独回答已确认写落在哪些故障域、网络分区时谁还能写，以及两个并发版本怎样裁决。同一个 Active-Passive 外形既可能异步复制、允许非零 RPO，也可能在返回 ACK 前同步到独立故障域；所谓 Active-Active 也可能只是不同 shard 各有一个单写者，并不意味着同一对象可以多主并发写。

选择复制方案时，应从业务允许的 History 和 ACK 合同向下推，而不是从产品标签向上猜：

```mermaid
flowchart TB
  S["先写业务契约"] --> P{"网络分区时<br/>两侧能否同时确认同一对象写入？"}
  P -->|"不能；守住单一历史"| D{"一个声明的故障域丢失后<br/>已确认写能否丢失？"}
  D -->|"不能，目标 RPO=0"| Q["跨独立故障域同步提交<br/>同步 Primary-Backup 或 Quorum SMR"]
  D -->|"可以，RPO>0"| A["异步 Primary-Backup / 日志重建<br/>把复制 lag 写进 ACK 与 RPO"]
  P -->|"可以；接受并发分叉"| M["多主接入<br/>显式冲突检测与业务合并"]
  Q --> O{"对象能否由一条<br/>有序命令流决定？"}
  O -->|"能"| R["复制日志 + 确定性状态机"]
  O -->|"不能"| X["分片 / 事务协议<br/>定义跨对象原子性"]
  M --> B["证明收敛与业务不变量<br/>不再宣称同一对象线性一致写"]
```

图中的 `RPO=0` 也必须带故障模型：同步到同一台宿主机上的两个进程，不能覆盖宿主机断电；同步到同一可用区的三个节点，也不能覆盖整区失联。副本数量只有投影到明确的故障域，才会变成可验收的冗余。

### 故障域边界必须覆盖数据面、控制面与共因

| 声明的故障域       | 可能一起失效的事实                         | 独立性至少要求什么                                    |
| ------------------ | ------------------------------------------ | ----------------------------------------------------- |
| 进程 / JVM         | 堆、线程、Safepoint、进程级配置            | 另一进程能独立恢复；不能据此宣称可抗宿主机故障        |
| 宿主机 / 本地介质  | 内核、供电、NIC、本地盘与 page cache       | 副本位于不同宿主机；完成快照不只存在故障机本地        |
| 机架 / 可用区      | ToR、供电、机房网络与区级控制组件          | 提交副本和恢复介质按拓扑分散，法定人数不集中在同一域  |
| 地域               | 区域网络、区域控制面和托管服务             | 同步跨地域提交，或公开异步复制的 lag、RPO 与恢复裁决  |
| 共享依赖与软件版本 | Kafka、ZooKeeper、KMS、DNS、错误配置和代码 | 分别分析依赖失效；版本/配置共因不能靠增加同构副本消除 |

因此故障域不是部署图上的彩色框，而是 ACK 合同的量词范围。例如“任一宿主机永久丢失后，所有 `QUORUM_COMMITTED` 操作仍可恢复”可以被注入故障和对账推翻；“我们有三个副本”则不能。本文后续选择的是这棵决策树中的单一权威写路径，并以停写守住不确定期，而不是覆盖所有高可用模式。

## 架构模型：单写者与热备副本

下面的参考模型只有一个节点对外产生权威副作用。Follower 虽然持续执行相同输入、生成快照并计算校验信息，但不接受业务写流量，因此它是 **Active-Passive hot standby**，而不是 Active-Active；这个标签只说明服务角色，实际持久性仍由 Kafka 写入 ACK、输入保留、checkpoint/outbox 与恢复合同共同决定。
