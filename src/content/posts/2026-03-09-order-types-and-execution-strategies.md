...
生产事件至少应包含 `orderId`、`clientOrderId`、订单版本、累计成交量、剩余量、状态原因和撮合序列。客户端重试应复用账户作用域内稳定的 `clientOrderId`，并通过查询或事件流消解超时后的未知结果。
...
- 是否把 price type、TIF、modifier 和 trigger 分字段建模；
- 是否保存交易所原始 order/fill IDs 与稳定的客户端幂等键；
- 是否按累计成交量处理重复和乱序事件；
- 是否把 timeout 视为 unknown，而不是 rejected；
- 是否等待 cancel acknowledgment 后才释放剩余数量；
- 是否按产品能力校验 Post-Only、Reduce-Only 和持仓模式；
- 是否明确跨场所请求没有原子提交和回滚；
- 是否将所有费率、价格保护和参数视为可版本化配置。
