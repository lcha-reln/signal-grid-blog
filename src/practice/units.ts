...
export const PRACTICE_UNITS: readonly PracticeUnit[] = [
  ...
  },
  {
    ...
    planCompatibility: "PLAN v0.6 只为 M04 冻结 GTC/IOC/FOK/POST_ONLY 的 ExecutionPolicy 结果合同，并把 price band、STP 与持久化继续留在后续单元；M01 价格时间优先、事件 batch、Golden corpus 与 evidence 合同不变。",
  ...
  },
  {
    ...
    planCompatibility: "PLAN v0.6 只为 M04 冻结 GTC/IOC/FOK/POST_ONLY 的 ExecutionPolicy 结果合同，并把 price band、STP 与持久化继续留在后续单元；M02 可寻址生命周期、10/34 corpus、M02H1 与 evidence 合同不变。",
  ...
  },
  {
    ...
    planCompatibility: "PLAN v0.6 只为 M04 冻结 GTC/IOC/FOK/POST_ONLY 的 ExecutionPolicy 结果合同，并把 price band、STP 与持久化继续留在后续单元；M03 独立参考模型、生成历史、反例缩小、matching-0.1.0 与 evidence 合同不变。",
  ...
  },
  {
    ...
    delivers: [
      ...
      "GTC 完全继承 M01–M03 的 Accepted → Trade* → Rested? 语义；IOC、FOK 与 POST_ONLY 复用同一价格时间匹配循环和 maker-price 成交合同",
    ...
    gate: [
      "M00–M03 累计回归、matching-reference 独立边界和 matching-0.1.0 固定 evidence 保持 PASS，既有 GTC 历史的事件、盘口和 digest 不被重写",
    ...
    evidence: [
      ...
      "M00–M03 累计回归、GTC 兼容、验证优先级、IOC 数量分区、FOK/Post-only 不变性和晚到 Cancel 报告",
