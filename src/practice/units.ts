...
export const PRACTICE_UNITS: readonly PracticeUnit[] = [
  ...
  },
  {
    ...
    delivers: [
      ...
      "lifecycle registry 的 RESTING entry、订单簿节点与 FILLED/CANCELED tombstone 共同形成可检查但仍只驻留内存的生命周期状态",
    ],
    freezes: [
      "Place 的 M00 验证失败或 Cancel 的 instrumentId/orderId 验证失败只产生 Rejected(code, field)，不分配 acceptedSequence、不占用 orderId 且不修改状态",
      "已接受 orderId 在当前 engine 生命周期内只能首次使用一次；ACTIVE、FILLED 或 CANCELED 上的重复 Place 产生 PlaceRejected(DUPLICATE_ORDER_ID)",
      "有效但从未接受的 orderId 撤单产生 CancelRejected(ORDER_NOT_FOUND)，且不创建 tombstone；之后的首次合法 Place 仍可接受",
      "ACTIVE 撤单产生唯一 Canceled(sequence, orderId, side, priceTicks, canceledQuantityLots)，删除精确剩余量且不消耗 acceptedSequence",
    ...
    gate: [
      ...
      "冻结 10 场景、34 命令 lifecycle corpus，覆盖未知撤单、活动重复 ID、中间/队首撤单、部分成交余量、迟到/重复撤单、终态 ID 复用和 SELL 镜像",
    ...
    interaction: [
      "L1 状态矩阵：在揭示结果前判断 UNSEEN、ACTIVE、FILLED 与 CANCELED 上的 Place/Cancel disposition",
    ...
    evidence: [
      ...
      "PUBLISHED 前由 complete tag CI 生成 manifest、artifact SHA-256 和明确 limitations；CONTRACTED 阶段不预造 evidence",
    ],
