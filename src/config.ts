...
export const SERIES: readonly SeriesDefinition[] = [
    ...
    stages: [
      {
        ...
        title: "可靠低延迟传输",
        description: "从身份与位置模型进入 SBE 协议、发送、接收、可靠 UDP、拓扑和 Media Driver 运维。",
        fromOrder: 5,
      },
      {
        index: "02",
  ...
  },
  {
    ...
    description: "围绕中心化交易所，从产品、订单语义与交易前风控一路走到撮合、OMS、行情重建、仓位、期货结算、账本、保证金和清算，建立业务规则与系统状态机之间的映射。",
    prerequisite: "理解基础现货交易和百分比、盈亏计算；了解事件驱动架构更佳。",
    outcome: "能够解释订单从准入、撮合、私有回报到结算入账的完整生命周期，说明公开行情与 OMS 如何从权威事件恢复，并把产品规则转成可测试、可审计的状态、风控、数据与账务约束。",
    color: "amber",
    index: "02",
    stages: [
      {
        ...
        eyebrow: "MARKET MAP",
        title: "市场与产品",
        description: "先建立产品、参与者、现金流和系统边界，再把合约规格、交易日与市场状态收敛为版本化主数据。",
        fromOrder: 10,
      },
      {
        index: "02",
        ...
        title: "订单、风控、撮合与行情",
        description: "从订单契约与原子准入追到订单簿、撮合、OMS 和私有回报，再用快照、增量、序列号与校验恢复公开行情。",
        fromOrder: 30,
      },
      {
        index: "03",
        ...
        title: "仓位、结算与账本",
        description: "连接仓位、每日盯市与交割、资金费率、双重记账、余额不变量和风险定价。",
        fromOrder: 60,
      },
      {
        index: "04",
  ...
  },
  {
    ...
    description: "围绕单写者有状态服务，从 WAL、本地持久化与崩溃恢复，推进到时间、一致性、复制与 fencing，再连接消息连续性、跨系统副作用、过载控制、一致检查点、灾备、滚动升级与故障证明。",
    prerequisite: "理解基础分布式系统与事件驱动概念；不要求预先掌握 WAL、Kafka、Aeron 或 ZooKeeper。",
    outcome: "能够定义故障模型、确认点与持久化边界，区分墙钟、逻辑顺序与权威提交，设计幂等副作用和过载降级协议，建立一致检查点、PITR 与安全升级边界，并用可重放故障实验给出 RPO/RTO 与恢复正确性的证据。",
    color: "coral",
    index: "03",
    stages: [
      {
      ...
      },
      {
        ...
        title: "时间、共识与协调",
        description: "先区分墙钟、逻辑顺序与故障检测，再用历史模型辨别一致性合同，最后理解多数派日志、任期、协调状态、Lease 与 fencing。",
        fromOrder: 25,
      },
      {
        index: "03",
        ...
        title: "分布式日志与消息连续性",
        description: "连接 Kafka 日志、消费位置、应用序列号、跨系统副作用与过载反馈环。",
        fromOrder: 50,
      },
      {
        index: "04",
        ...
        title: "检查点、灾备与验证",
        description: "把一致检查点、PITR、灾难恢复、混合版本升级和故障验证收敛为可演练、可回滚、可证明的恢复链。",
        fromOrder: 80,
      },
    ],
  },
    ...
    stages: [
      {
        ...
        title: "并发正确性与性能证据",
        description: "先证明线程间协议正确，再建立能够识别尾延迟、饱和与测量偏差的证据链。",
        fromOrder: 10,
      },
      {
        index: "02",
      ...
      },
      {
        ...
        title: "事件通路与执行模型",
        description: "用 Disruptor 与 Agrona 落地有界通路、单写者、批处理、背压和 Agent 循环。",
        fromOrder: 70,
      },
    ],
  },
